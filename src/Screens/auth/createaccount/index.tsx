...
interface CreateAccountProps extends NativeStackScreenProps<
  any,
  "CreateAccount"
> {}
...
export function CreateAccount(props: CreateAccountProps) {
  ...
  const signup: SubmitHandler<SignUpFormData> = async (formData: SignUpFormData) => {
    if (!checked) {
      Toast.show({
        type: "error",
    ...
    >
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <Controller
          name="fullName"
          control={control}
          render={({ field: { onChange, value, onBlur } }: { field: { onChange: (value: string) => void; value: string; onBlur: () => void } }) => (
            <TextField
              value={value}
              ...
              LeftAccessory={() => (
                <WithLocalSvg style={{ marginLeft: 10 }} asset={Images.user} />
              )}
